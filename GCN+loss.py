...
for i in range(0, 10):
    ...
    with torch.no_grad():
        ...
        for data in test_loader:  # Iterate in batches over the training/test dataset.
            out, _ = model(data.x.to(device), data.edge_index.to(device), data.edge_weight.to(device), data.batch.to(device))
...
with open("GCN+loss.txt","a") as file:
